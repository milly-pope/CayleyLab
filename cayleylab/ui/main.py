...
def select_generators(configured):
    ...
    # For Wreath products, don't offer defaults (too complex)
    if configured.name == "Wreath":
        print("\nFor general wreath products, please specify generators as composite generators.")
        print("Examples: 't' (move), 'a' (toggle lamp at identity offset), 'ta' (move then toggle)")
        from ..groups.wreath import CompositeGen
        
        # Just ask for custom composites
        gen_names = {}
        print("\nEnter generator specifications (space-separated names):")
        print("Common: t (move), a/b/c (lamp toggles), T (inverse move)")
        
        composites = []
        count = 0
        while count < 10:
            line = input(f"  Generator {count+1} (or empty to finish): ").strip()
            if not line:
                break
            composites.append(CompositeGen(line, []))
            count += 1
        ...
        if composites:
            ...
            print("No generators specified, using defaults.")
            return configured.default_generators()
    
    all_gens = configured.default_generators()
    
    # For Z_RestrictedWalk, just use all generators (they're automatically paired with inverses)
...
def configure_group(group):
    ...
    if group.name in ("Z^2", "D∞"):
        ...
        print("Inverses are automatic.\n")
        steps_str = input("Generator steps [a=1]: ").strip() or "a=1"
        
        # Parse steps: "a=3, b=2" → {'a': 3, 'b': 2}
        steps = {}
        try:
            for part in steps_str.split(','):
                ...
                if '=' in part:
                    name, val = part.split('=')
                    steps[name.strip().lower()] = int(val.strip())
        except:
            print("Invalid format. Using default: a=1")
            steps = {'a': 1}
        
        return group.parse_options({"steps": steps})
        ...
        if walking.lower() != 'z' and walking.lower().endswith('z'):
            ...
            try:
                ...
                if n > 1:
                    num_blocks = n
                    print(f"Walking on {n}Z creates {n} positions")
                    print(f"Specify {n} lamps or same lamp at all positions\n")
            except ValueError:
                pass
        ...
        if ',' in base:
            ...
            if num_blocks is not None and lamp_count != num_blocks:
                print(f"Warning: {walking} expects {num_blocks} positions, you gave {lamp_count} lamps")
                proceed = input("    Continue anyway? (y/n) [n]: ").strip().lower()
                if proceed != 'y':
                    return None
        elif num_blocks is not None:
            print(f"Using same lamp at all {num_blocks} positions")
        ...
        try:
            ...
            print(f"Error: Invalid specification: {e}")
            return None
...
def main_menu():
    ...
    from ..groups.Z_restricted_walk import Z_RestrictedWalk
...
def dead_end_mode(group):
    ...
    from ..core.deadends import analyze_dead_ends, print_dead_end_results
