...
def normalise_tape(tape_dict, top_adapter, base_adapter):
    # Convert tape dict to normalised sorted tuple, dropping identity entries
    # Returns sorted tuple of ((addr, val), ...) where val ≠ base.one()
    items = []
    #remove idenitty values and convert to list of (addr, val) pairs
    for addr, val in tape_dict.items():
        if not base_adapter.is_one(val): # Only include non-identity lamps, the is_one function of each adapter determines what the identity is for that lamp type
            items.append((addr, val))
    # Sort by address (using top_adapter's pretty for consistent ordering)
    try:
        items.sort()
    except TypeError:
...
class ToggleGen:
    ...
    def apply(self, state):
        ...
        # Re-normalise
        new_tape = normalise_tape(tape, self.top, self.base)
        return (d, new_tape)
...
class WreathProduct:
    ...
    def __init__(self, base_adapter=None, top_adapter=None, 
        ...
        self.base = base_adapter 
        self.base_adapters = base_adapters  
        self.top = top_adapter
    ...
    def default_generators(self):
        #If user dosent enter generators and selects defult
        gens = []
