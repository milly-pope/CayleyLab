...
# We need adapters so that the tape implementation logic can be generic
# Essentially each adapter tells the tape code how to store and update a lamp value at each site
# For each adapter, we implement the group operations (one, multiply, inverse) and a pretty printer. Where one() gives the identity element, multiply(a,b) gives the product of a and b, and inverse(a) gives the inverse of a. The pretty() method turns an element into a nice string for visualization.
# We also implement a default_increments() method which gives a set of "increment" generators for the base group, which the lamplighter code can use to automatically generate lamp toggling generators
...
def get_base_adapter(spec):
    ...
    # Check for direct product notation
